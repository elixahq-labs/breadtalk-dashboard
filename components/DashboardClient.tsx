...
export default function DashboardClient({ rawData }: { rawData: any[] }) {
  // 1. STATE SLICERS
  ...
  const { netRevenue, totalBills, aov, discountRateTB, wasteQty, cancelRate, trendData, paymentData, topSalesByGroup, topWasteByGroup, agingTickets, pendingStats, gapData } = useMemo(() => {
    ...
    filteredData.forEach(row => {
      ...
      if (!trendMap[day]) {
        trendMap[day] = { day, revenue: 0, target: 0, discountAmt: 0, waste: 0, countBill: 0, cancel: 0 };
      }
      ...
      if (type === 'Payment') {
        ...
        const paymentSales = parseNum(row['Sales']);
        paymentMap[pType] = (paymentMap[pType] || 0) + paymentSales;
      }
      ...
      if (sku) {
        ...
        if (!gapMap[key]) {
          gapMap[key] = { group: groupName, sku, name: row['Product-Name'], open:0, process:0, import:0, export:0, sales:0, waste:0, stock:0, gap:0 };
        }
    ...
    const tData = Object.values(trendMap).map(d => ({
      ...d, discount: d.revenue > 0 ? (d.discountAmt / d.revenue) * 100 : 0
    ...
    const tSalesGroup = Object.keys(salesMapByGroup).map(group => ({
      group,
      items: Object.values(salesMapByGroup[group]).sort((a: any, b: any) => b.qty - a.qty).slice(0, 5)
    })).filter(g => g.items.length > 0);

    const tWasteGroup = Object.keys(wasteMapByGroup).map(group => ({
      group,
      items: Object.values(wasteMapByGroup[group]).sort((a: any, b: any) => b.qty - a.qty).slice(0, 5)
    })).filter(g => g.items.length > 0);
    ...
    return {
      netRevenue: rev,
      totalBills: countBills,
      aov: countBills > 0 ? rev / countBills : 0,
      discountRateTB: rev > 0 ? (totalDiscount / rev) * 100 : 0,
      wasteQty: waste,
      cancelRate: countBills > 0 ? (cancelBills / countBills) * 100 : 0,
      trendData: tData,
      paymentData: pData,
      topSalesByGroup: tSalesGroup,
      topWasteByGroup: tWasteGroup,
  ...
  // ==========================================
  // 6. GIAO DIỆN (ĐÃ TỐI ƯU MOBILE & TABLET)
  // ==========================================
  return (
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3 w-full">
          ...
          <div className="flex flex-row items-center justify-between space-x-2 border border-gray-300 rounded-md bg-white p-1 w-full md:w-auto overflow-hidden">
            ...
            <input 
              ...
              className="p-1 text-sm outline-none bg-transparent text-gray-700 cursor-pointer w-full"
            />
            ...
            <input 
              ...
              className="p-1 text-sm outline-none bg-transparent text-gray-700 cursor-pointer w-full pr-2"
            />
          </div>
            ...
            <table className="w-full text-sm text-left whitespace-nowrap">
              <thead className="sticky top-0 bg-white">
                <tr className="text-gray-500 border-b border-gray-100">
                  <th className="pb-2 font-medium px-1">SKU</th>
                  <th className="pb-2 font-medium px-1">Sản phẩm</th>
                  <th className="pb-2 font-medium px-1 text-right">Qty</th>
                </tr>
              </thead>
              <tbody>
                {topSalesByGroup.map((g, gIdx) => (
                  <React.Fragment key={gIdx}>
                    <tr className="bg-blue-50 border-y border-gray-200">
                      <td colSpan={3} className="py-2 px-2 font-bold text-blue-800 uppercase text-xs">Group: {g.group}</td>
                    </tr>
                    {g.items.map((item: any, i: number) => (
                      <tr key={i} className="border-b border-gray-50 hover:bg-gray-50">
                        <td className="py-2 px-2 text-xs sm:text-sm">{item.sku}</td>
                        <td className="py-2 px-1 font-medium truncate max-w-[150px] sm:max-w-[200px]" title={item.name}>{item.name}</td>
            ...
            <table className="w-full text-sm text-left whitespace-nowrap">
              ...
              <tbody>
                {topWasteByGroup.map((g, gIdx) => (
                  <React.Fragment key={gIdx}>
                    <tr className="bg-red-50 border-y border-gray-200">
                      <td colSpan={3} className="py-2 px-2 font-bold text-red-800 uppercase text-xs">Group: {g.group}</td>
                    </tr>
                    {g.items.map((item: any, i: number) => (
                      <tr key={i} className="border-b border-gray-50 hover:bg-gray-50">
                        <td className="py-2 px-2 text-xs sm:text-sm">{item.sku}</td>
              ...
              {agingTickets.map((ticket, idx) => {
                ...
                return (
                  <tr key={idx} className={`border-b border-gray-100 hover:bg-gray-50 ${isCritical ? 'bg-red-50 text-red-700 font-medium' : 'text-gray-700'}`}>
                    ...
                    <td className="py-2 px-2 text-center text-xs sm:text-sm">{formatUS(ticket.qty)}</td>
                    <td className="py-2 px-2 text-center flex items-center justify-center gap-1 text-xs sm:text-sm">
                      {formatUS(ticket.aging)} {isCritical && <AlertTriangle size={14} className="text-red-500" />}
                    </td>
                  </tr>
                );
              })}
          ...
          <table className="w-full text-sm text-left whitespace-nowrap">
            <thead className="sticky top-0 bg-white z-10 shadow-sm">
              <tr className="text-gray-500 border-b border-gray-200">
                ...
                <th className="pb-3 px-2 font-medium text-right">Open</th><th className="pb-3 px-2 font-medium text-right">Process</th>
                <th className="pb-3 px-2 font-medium text-right">Import</th><th className="pb-3 px-2 font-medium text-right">Export</th>
                <th className="pb-3 px-2 font-medium text-right">Sales</th><th className="pb-3 px-2 font-medium text-right">Waste</th>
                <th className="pb-3 px-2 font-medium text-right">Stock</th><th className="pb-3 px-2 font-bold text-right text-gray-800">GAP</th>
              </tr>
            </thead>
            <tbody>
              {gapData.map((row, idx) => (
                <tr key={idx} className="border-b border-gray-100 text-gray-700 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-2 font-medium truncate max-w-[150px] sm:max-w-[200px]" title={row.name}>{row.name}</td>
                  <td className="py-3 px-2 text-right text-xs sm:text-sm">{formatUS(row.open)}</td>
                  <td className="py-3 px-2 text-right text-xs sm:text-sm">{formatUS(row.process)}</td>
                  <td className="py-3 px-2 text-right text-xs sm:text-sm">{formatUS(row.import)}</td>
                  <td className="py-3 px-2 text-right text-xs sm:text-sm">{formatUS(row.export)}</td>
                  <td className="py-3 px-2 text-right text-xs sm:text-sm">{formatUS(row.sales)}</td>
                  <td className="py-3 px-2 text-right text-xs sm:text-sm">{formatUS(row.waste)}</td>
